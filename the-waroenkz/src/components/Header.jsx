...
export default function Header({ rightContent, subtitle }) {
  return (
    <header className="h-16 border-b-2 border-border bg-sidebar flex items-center justify-between px-6 shrink-0 z-20">
      <div className="flex items-center gap-4">
        <Link to="/" className="font-serif text-xl md:text-2xl font-bold tracking-tight text-foreground" aria-label="Serif POS System">
          Serif<span className="text-gold-dark">.</span> POS
        </Link>
        {subtitle && (
          <>
            <span className="h-6 w-0.5 bg-muted hidden md:block" aria-hidden="true"></span>
            <span className="font-mono text-sm font-semibold uppercase tracking-caps text-muted hidden md:block">{subtitle}</span>
          </>
        )}
      </div>
